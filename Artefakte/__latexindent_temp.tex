...
\begin{document}
	...
	\newpage
	\section{System-Sequenz-Diagramm}
	\begin{tikzpicture}
		\begin{umlseqdiag}
			\umlactor[class=Verkaeufer]{A}
			\umlobject[class=System, x=10]{B}
			\begin{umlcall}[op={Fügt ausgewählten Projekt Offerte hinzu}]{A}{B}
			\begin{umlcall}[op={Fragt nach Titel sowie Baubranche}]{B}{A}
			\end{umlcall}
		\end{umlcall}
			\begin{umlcall}[op={Gibt Titel und Baubranche an}]{A}{B}
			\begin{umlcall}[op={Erstellt neue Offerte mit Titel und Baubranche}]{B}{A}
			\end{umlcall}
		\end{umlcall}
			\begin{umlcall}[op={Fügt der Offerte einen Standartisierten Artikel hinzu}]{A}{B}
			\end{umlcall}
			\begin{umlcall}[op={Informiert über existierende Artikel}]{B}{A}
			\end{umlcall}

			\begin{umlcall}[op={Wählt Artikel aus mit Stückzahl und Rabatt}]{A}{B}
			\end{umlcall}
			\begin{umlcall}[op={Fügt Artikel mit Stückzahl und Rabatt der Offerte hinzu}]{B}{A}
			\end{umlcall}

			\begin{umlcall}[op={Fügt der Offerte einen manuellen Artikel hinzu}]{A}{B}
			\end{umlcall}
			\begin{umlcall}[op={Fragt ob neuer oder existierender manueller Artikel}]{B}{A}
			\end{umlcall}

			\begin{umlcall}[op={Wählt die Option existierender manueller Artikel}]{A}{B}
			\end{umlcall}
			\begin{umlcall}[op={Informiert über existierende manuelle Artikel}]{B}{A}
			\end{umlcall}

			\begin{umlcall}[op={Wählt manuellen Artikel aus mit Stückzahl und Rabatt}]{A}{B}
			\end{umlcall}
			\begin{umlcall}[op={Fügt manuellen Artikel mit Stückzahl und Rabatt hinzu}]{B}{A}
			\end{umlcall}

			\begin{umlcall}[op={Schliesst die Offertenerstellung ab}]{A}{B}
			\end{umlcall}
			\begin{umlcall}[op={Speichert Daten und berechnet Total}]{B}{A}
			\end{umlcall}
